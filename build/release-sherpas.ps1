...
$versionIncrement = Get-Content "$PSScriptRoot\version.sherpas.increment" -Raw
$versionIncrement = $versionIncrement -as [int]
$versionIncrement++

$version = Get-Content "$PSScriptRoot\version.sherpas" -Raw

$version = $version.Replace("{incremental}", $versionIncrement)

Write-Host "Building PnP.Framework version $version"
dotnet build $PSScriptRoot\..\src\lib\PnP.Framework\PnP.Framework.csproj --configuration Release --no-incremental --force /p:Version=$version
